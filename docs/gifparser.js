...
const GIFCpy = function (_gif) {

    var gif = {                      // the gif image object
        onload: _gif.onload,       // fire on load. Use waitTillDone = true to have load fire at end or false to fire on first frame
        onerror: _gif.onerror,       // fires on error
        onprogress: _gif.onprogress,       // fires a load progress event
        onloadall: _gif.onloadall,       // event fires when all frames have loaded and gif is ready
        paused: _gif.paused,      // true if paused
        playing: _gif.playing,      // true if playing
        waitTillDone: _gif.waitTillDone,       // If true onload will fire when all frames loaded, if false, onload will fire when first frame has loaded
        loading: _gif.loading,      // true if still loading
        firstFrameOnly: _gif.firstFrameOnly,      // if true only load the first frame
        width: _gif.width,       // width in pixels
        height: _gif.height,       // height in pixels
        frames: _gif.frames,         // array of frames
        comment: _gif.comment,         // comments if found in file. Note I remember that some gifs have comments per frame if so this will be all comment concatenated
        length: _gif.length,          // gif length in ms (1/1000 second)
        currentFrame: _gif.currentFrame,          // current frame. 
        frameCount: _gif.frameCount,          // number of frames
        playSpeed: _gif.playSpeed,          // play speed 1 normal, 2 twice 0.5 half, -1 reverse etc...
        lastFrame: _gif.lastFrame,       // temp hold last frame loaded so you can display the gif as it loads
        image: _gif.image,       // the current image at the currentFrame
        playOnLoad: _gif.playOnLoad,       // if true starts playback when loaded

        // Functions
        load: _gif.load,    // call this to load a file
        cancel: _gif.cancel, // call to stop loading
        play: _gif.play,       // call to start play
        pause: _gif.pause,      // call to pause
        seek: _gif.seek,       // call to seek to time
        seekFrame: _gif.seekFrame,  // call to seek to frame
        togglePlay: _gif.togglePlay, // call to toggle play and pause state
    };
